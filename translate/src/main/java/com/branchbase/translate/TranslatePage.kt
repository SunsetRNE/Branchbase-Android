 ...
 * | 文件 | 职责 |
 * |------|------|
 * | `01-core.js` | 配置、状态机、与原生侧的异步桥、批量队列、快捷设置与可见视口通道 |
 * | `02-dom.js`  | 段落收集/过滤/跳过、译文插入、视口观察、候选统计与清空 |
 * | `03-fab.js`  | 可移动悬浮球：拖拽 / 吸附可见带 / 状态灯 / 提示 |
 * | `04-panel.js`| 翻译工具选项菜单：进度数据、快捷设置、关闭与点外收起 |
 * | `05-boot.js` | 启动：恢复上次开关、自动翻译、显示方式/样式/语言的写入口、滚动兜底 |
 *
 * 拆分的收益不是「文件多好看」，而是：改判定规则只动 02、改悬浮球只动 03、
 * 改面板只动 04，且每个文件都能单独通读（合成一个 600 行文件之后没人会去读完它）。
 * 顺序是**显式契约**：01 建命名空间，02/03/04 挂方法，05 最后启动。
 */
object TranslatePage {

    /** 样式（译文卡片、悬浮球、工具面板、状态提示）。 */
    ...
    val JS_ASSETS: List<String> = listOf(
        ...
        "translate/02-dom.js",
        "translate/03-fab.js",
        "translate/04-panel.js",
        "translate/05-boot.js",
    )
