 ...
 *
 * 窗口约 **2.5 小时**（08:00Z 首个受影响、10:24Z 收敛）。但更要紧的是**它不是一个「窗口」**：
 * 每 4 分钟采一次能看到同一个 release、同一个端点的值在 `0` 与 `3` 之间**反复横跳**
 * （09:42 恢复 → 09:47 又变回 0 → 10:15 恢复 → 10:20 又全部变 0 → 10:24 起稳定），
 * 这是 GitHub 后端**多副本数据不一致**，不是按 release 年龄过期的缓存。
 * 单点采样会把它误读成窗口 —— 第一版结论就是这么错的。
