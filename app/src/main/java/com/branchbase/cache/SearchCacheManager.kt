...
class SearchCacheManager(
    ...
    companion object {
         ...
         *
         * ⚠️ 键是**稳定内部键**，不是界面文案：
         * 搜索类走 `SearchType.cacheKey`（`code` / `repos` / …，见 `ui/search/SearchType.kt`），
         * 页面类走 `PageCache` / `PreloadStore` 里那些常量。
         * 两者都**不许**跟着界面语言变 —— 缓存是落库的，键一变旧行就全成孤儿。
         *
         * 改前这里是 7 个中文字面量在派发（`"代码"` / `"仓库"` …），其中 `"Issues"`
         * 还是个**大小写错的死分支**：搜索类型实际叫「议题」，于是议题搜索的 TTL
         * 一直落在 `else`（30 分钟），而不是它想要的 15 分钟。
         */
        fun ttlFor(type: String): Long = when (type) {
            "code" -> 60 * 60 * 1000L          // 1 小时（代码搜索速率限制最严，缓存更久）
            "repos" -> 30 * 60 * 1000L         // 30 分钟
            "分支" -> 30 * 60 * 1000L          // 30 分钟（分支列表变化不频繁）
            "README" -> 30 * 60 * 1000L        // 30 分钟（README 渲染 HTML 缓存）
            // 议题 / 拉取请求 / 用户搜索：变化快，且都有速率限制
            "issues", "pulls", "users" -> 15 * 60 * 1000L // 15 分钟
            "commits" -> 10 * 60 * 1000L       // 10 分钟（提交搜索也有速率限制）
            "topics" -> 60 * 60 * 1000L        // 1 小时（主题变化慢）
