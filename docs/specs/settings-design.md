...
设置树多了一行 **设置主页 → 「外观」分组 → 语言**（`SubPage.Language`），
二级页是 `ui/settings/LanguageScreen.kt` 的 `LanguageScreen()`。
它是[界面语言体系](i18n-migration.md)的入口 —— 语言清单、默认语言、切换为什么不重建 Activity
都在那份文档里，本节只记**它作为设置行**的规范约束。

| 项 | 取值 | 依据 |
|---|---|---|
| 行型 | **`NavRow`**（进二级页） | 选项数随支持语言增长，内联分段控件放不下；二级页是单选列表 |
| 条件显示 | API 33+ **且**清单里至少两种语言；否则**整行不出现** | §3.2.1 对「仓库凭据」是同一条道理：禁用行必须给「怎么才能开」的出路，而这里的出路是换台新系统手机，不属于设置页能代办的事。判定收在 `languagePickerAvailable()` 里，它同时覆盖这两个条件 |
| 值列（跟随系统） | `跟随系统` | §6.1 用语表**逐字**规定（资源 `language_follow_system`） |
| 值列（指定语言） | 该语言的**母语自称**（`English`） | 与二级页里的名称同源 —— 同一含义在整棵树里只有一种写法，不能值列写 `English`、列表里写「英语」 |
| 名称 | **母语自称**（endonym）：`English` 而不是「英语」 | 用户在看不懂当前界面语言时，也必须能认出自己那一行；反过来会让「界面已经是看不懂的语言」的用户彻底卡死 |
| 说明 | 当前界面语言的**他称**（exonym）：界面是中文时写「英语」；两者相同时**不给说明** | §6.2 禁止用说明复述名称 |
| 二级页分组标签 | **不写** | §3.2 禁止分组名与行名重复（整页只有这一组） |

**一条不写进规范的边界**：值列在清单里找不到该标签时（清单变了、或系统还原了一个本包没有的语言）
回落成「跟随系统」，而不是把裸语言标签（`en-US`）丢到值列上 —— 裸标签对用户没有意义。

`AppLanguageTest` 6 例钉住上面的**关系**（名称用母语自称、说明用他称、两者相同时不给说明），
刻意**不比对具体译文**：「中文」还是「简体中文」取决于 JDK 的 CLDR 版本，
钉死它等于给自己埋一个跟 JDK 升级一起红的假红。
