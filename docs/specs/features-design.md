...
四段：待处理 / 进行中 / 常用仓库 / 最近活动（`ui/home/HomeScreen.kt`）。

- **这一栏有两个数据源，选谁只看「有没有置顶」**（纯函数 `FrequentRepoSource`，`ui/home/FrequentRepoStore.kt`）：
  - **没置顶过**（键不存在，或把置顶全部取消）→ 默认态：收藏（星标）仓库，服务端按最近星标倒序取前 5 ——
    `/user/starred?per_page=100&sort=created&direction=desc`（常量 `STARRED_RECENT_PATH`），
    与 1.1.8 及以前一致；缓存键刻意**沿用 1.1.8 的 `starred_repos`**，升级上来的用户首帧零闪动。
  - **置顶过** → 只显示常用仓库：候选集 = 我能用的仓库
    `/user/repos?per_page=100&affiliation=owner,collaborator,organization_member&sort=pushed&direction=desc`
    （常量 `MY_REPOS_PATH`）—— 账号自身持有的 + 被协作/有权限的 + 所属组织与团队里的仓库。
  两个源的查询串都**显式写全**参数（GitHub 的默认值恰好就是这些，靠默认值过日子的话默认一变就静默变少/取错序）。
  **排序必须留在服务端**：`per_page=100` 只取第一页，客户端排序会退化成「先按仓库名取一页再排序」，
  名字靠后的活跃仓库根本进不了这一页。两个源各有缓存键（`my_repos` / `starred_repos`），
  共用会让切源后的第一帧拿另一份 JSON 渲染（都是「闪一下又变」，且顺序来源都不对）。
- **私有 / 复刻徽标**：仓库名右侧的描边小标签（`RepoBadges`，首页卡片与管理页列表行共用）。
  候选集里混着组织仓库、别人的协作仓库与复刻，用户需要一眼看出「这条链接谁能打开」「最近推送是不是上游的」。
  用描边 + 次级文字而不是彩底：这是仓库的**属性**、不是状态，彩底会和真正表示状态的色块抢读法。
- **「常用仓库」可自定义置顶**：长按标题行（或点右上角 `Tune` 图标）进管理页
  `ui/home/FrequentReposScreen.kt`，点一条 = 置顶到首页，**顺序就是点选的先后**，最多 5 个；
  选满后再点第 6 个给 Toast 说明原因 —— 静默无反应在真机上看起来就是「点击坏了」。
  长按**只挂标题行**：卡片区留给将来的「长按快捷操作」，现在占掉就是两者手势打架。
- **选中态不能只靠颜色**：未选中 1dp `Primer.Border`、选中 1.5dp `Primer.Blue500` + 6% 蓝底 +
  右侧对勾，且整行 `selectable(role = Role.Checkbox)`（读屏念「已选中 / 未选中」）——
  色弱用户与强光下的屏幕读不出纯色差（叠法与消息页多选行同源）。
- **存储的三种语义**（`ui/home/FrequentRepoStore.kt`，键 `home_frequent_repos`，**按账号分桶**）：
  键**不存在** = 从未自定义；**空数组** = 用户明确清空 → 首页**回落默认态**（显示收藏仓库最近 5 个，
  用户原话：「除非取消所有常用仓库的选择」）；数组非空 = 按用户顺序渲染，其中**已不在候选集里**的条目
  自动跳过（被移出协作者、组织权限变更）。两种「空」在**显示**上同义、在**存储**上仍分开
  （管理页要能区分「一个都没勾」和「从没进过这一页」，将来若给「清空过」换行为这信息也现成）。
  点一下立刻落盘、没有保存按钮。
  唯一的例外：用户**确实置顶了**、但那些仓库全都不在候选集里 → 显示「暂无置顶仓库 + 长按标题可重新选择」，
  **不**回落收藏仓库（静默换成另一份列表等于用户的选择被无声覆盖）。
- 首页那块右上角原来的**刷新**图标换成了管理图标：每次回到前台 `LaunchedEffect(resumeTick)`
  都会重拉仓库，刷新只让它早 0.1 秒；要强制刷新去管理页（那里有刷新按钮）。
  标题图标从 `Star` 换成 `Folder` —— 这一栏现在有两种来源（默认态是收藏仓库、置顶态是「我能用的仓库」），
  Star 只在默认态说得通、置顶态就是错的，所以用被整个 App 当作「仓库」的中性图标。
  空态文案也跟着源走：默认态空了说「暂无星标仓库」，置顶态候选集空了说「暂无可用的仓库」。
- 规则收在纯函数里（`FrequentRepoSource` 选源、`FrequentRepoRules` 管显示/上限/点选），钉子
  `FrequentRepoRulesTest`（8 例：缺键取前 5 / 按点选序 / 取消全部置顶回落默认态 / 失去访问权限被过滤 /
  超限截断保留靠前 / 元信息不丢 / 满员仍可取消 / 重勾排到末尾）与 `MyReposSourceTest`
  （6 例：三类 `affiliation` 显式写出 / 排序在服务端 / 未置顶读收藏、置顶读我能用的仓库且两键不同 /
  收藏按最近星标取一页 / 私有与复刻可解析 / 坏 JSON 与缺字段退化成空候选集与不亮的徽标）。
