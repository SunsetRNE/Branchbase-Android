...
消息页的列表是**行 + 1dp 分隔线**（1.0.76 从「卡片流」改过来）：一条通知 = 一行。
行布局与多选语义在 `ui/notification/`。

> **为什么不再是卡片**：浅色板里 `canvas` 与 `canvasSubtle` **都是纯白**，卡片一直只靠那圈灰边
> 撑着；一屏十几个盒子正是「空格子」观感的来源。深色下 `canvasSubtle #161B22` 与页面底本来就有
> 层次差，改成行之后两套主题都成立。**不加投影** —— 通知列表密度高，投影会糊成一片（这条从
> 卡片时代沿用）。

### 视图模式（平铺 / 按仓库 / 按主题 / 两级）**只有一份真源**

显示模式有**两个入口**：消息页右下角筛选面板、设置 → 通知 → 通知显示模式。
两个页面各自 `remember { readNotifLayout(context) }` 是错的 —— 谁后改都不会通知对方：

- 在设置页选了「平铺」，退回消息页看到的仍是进来时那份（例如「按仓库分组」）→
  用户看到的是「选了平铺，列表还是按仓库分组」，也就是**设置不生效**；
- 更麻烦的是它只在「消息页还活着」时复现（tab 切走再切回来页面重建，又是对的），
  于是变成「偶尔不生效」，最难查的那类。

所以显示模式走 `NotifLayoutRuntime`（`MutableStateFlow`，照 `ui/theme/ThemeRuntime` 的形状）：
两个入口都读它、都写它，落盘仍是 `writeNotifLayout`。页面上用 `collectAsState()` 订阅，
改完立刻一致，不需要重建页面。

> 同理，**分类计数与列表必须走同一个纯函数**（`notifCategoryBase`）：各写一遍过滤，
> 迟早出现「面板上写着 12 条、列表只有 11 行」这种对不上的账（已丢弃的条目就是这么漏的）。
...
| 未读竖条用 `matchParentSize` + `drawBehind` **overlay 绘制**（4dp） | 作为 flex 子项时未读行比已读行少 4dp 正文宽度，同一标题会换行到不同位置 |
| **未读行不铺底色**，未读只由「竖条 + 粗标题 + 尾点」表达 | 未读常是多数，每行都带底色等于整片都「被高亮」—— 高亮失去对比对象就不再是高亮。已读行保持干净白底，有没有未读一眼可数。**真机复看后的修正**：原型阶段给未读铺了 4% 蓝底，真机上反而更难分 |
| 选中行 = 6% 蓝底 **+ 1.5dp `BorderControl` 描边** | 只差 6% 的底在浅色屏 / 强光下几乎看不出哪几行被选中，边框是第二信号 |
| 展开控件是**无边框**的蓝色文字 + 箭头；展开明细用三级文字色 + 左侧导轨 | 带边框时它压在标题下、每折都出现一次，读起来像「第二个标题」—— 它是动作，不是内容。父行不再铺底后，明细才真正「从属」于那一行汇总 |
...
| 原因标签只在 `reasonHighSignal` 时渲染 | 一屏 8 个「CI 运行结果」只是重复占位，结论已经在标题里。文案不丢：长按动作面板仍读 `reasonLabel` |
| 标签与行首图标用 `TintRole.textColor()`，**不是** `color()` | `color()` 是填充色，压在自己的 12% 浅底上过不了 AA：WARNING 2.64 / DANGER 3.85 / ACCENT 4.38。文字角色实测 6.25 / 5.08 / 6.44 / 6.03（`ThemeContrastTest` 钉住浅深两套） |
| 骨架屏**不得有卡片壳** | 骨架与真实行的结构必须一起改，否则加载完会抖一下（看起来是一叠卡片、结果是分隔线列表） |

### CI 通知折叠：一屏 8 条一模一样的行收成 1 行

真机上连着 8 条 `Build workflow run failed for main branch`：同仓库、同分支、同一天，
标题**逐字相同**。逐条渲染读不出「今天挂了几次」，也不像消息列表 —— 与动态页
「最近 30 条里 28 条是 PushEvent」是同一个问题，口径照抄那边的 `collapsePushes`。

| 规则 | 说明 |
|------|------|
| 折叠键 | 同一仓库 + 同一 `subject.type` + 同一工作流 + 同一分支 + **同一天（本地时区）**，且 reason 必须是 `ci_activity` |
| 只在**相邻**条目之间折叠 | 输入已按时间倒序；中间夹了别的通知就断开 |
| **不跨天** | 跨天会把「今天 3 次 + 昨天 5 次」写成 8 次 —— 那是在编造事实 |
| 工作流名与分支**从标题解析**（`parseCheckSuiteTitle`） | CheckSuite 常没有 `subject.url`；解析不出来就**不参与折叠**，宁可多几行也不猜 |
| `updatedAtMs <= 0` 判为不同天 | 否则会把一批「时间未知」的通知并成一组，凭空造出「今天连续失败 N 次」 |
| 组内保留**最新一次**为代表，`fold.runs` 留全部明细 | 行内「展开其余 N 次」可原地铺开，**信息一条不丢** |
| 单条 CI **不呈现**折叠（`isFolded` = `count > 1`） | 否则会渲染出「展开其余 0 次」这种不成立的说法。`fold` 仍携带组信息，合并时要用 |
| 未读徽标**按行**报数（`unreadRowCount`），且**先折叠再筛未读** | 组内只要有一条未读，整行就是一个未读点；反过来先筛未读，组里已读的那几条被抽走后折叠组不再成立，「8 条折成 1 行」会退回 3 行 —— 徽标报 3 而屏幕上只有一个点 |

**折叠行代表 N 条，所有写远端的地方都要打散** —— 这是本块最容易漏的接线：
...
| 面板「复制链接」「恢复未读」 | 同上 |
| 顶部「全部已读」 | 端点是一次性 `mark-all-read`，但**本地已读集合**要按 id 逐条写全 |

不展开的后果很具体：把一折标为已读后服务端还剩 7 条未读，刷新回来它们重新组成一个新折叠行
（表现为「标了已读没生效」）。**回滚则相反 —— 一律整行**（`rollingBackRows`）：一折 8 条只失败
1 条时按条回滚，会让这一行显示成「已读」而其中一条在服务端仍是未读，与
[批量失败](#批量失败只回滚失败的条目)要解决的「本地与远端不一致」是同一个问题。
批量 Toast 与撤销文案一律**按行**计数。
...
| 左滑（**仅「已完成」分类**） | **丢弃**：从归档移除并从此不再显示（见下） | 一律禁用 |
...
#### 「已完成」的滑动语义是**另一套**：左滑 = 丢弃

归档行的 `unread` 恒为 `false`（`ArchivedThread.toNotification`），
所以「左右滑 = 标记已读」在那个分类里是**空操作**：手势滑得出来、什么也不会发生。
现在「已完成」改成 **左滑 = 丢弃**，而且**只放开左滑**
（右滑在那个分类没有可执行的动作，放开它只会滑出一个空承诺）。

**丢弃**的语义（`NotifDiscard`，键 `notif_discarded`）：

| 维度 | 行为 | 为什么 |
|---|---|---|
| 远端 | **不动** | GitHub 没有「永久隐藏这一条」的接口：`DELETE /threads/{id}` 是「完成」，`subscription=ignore` 是「这个仓库/会话以后都别通知我」——都不是用户点「丢弃」的意思 |
| 本地归档 | **保留** | 撤销只需把 id 从丢弃集合里去掉；删了归档就找不回来了 |
| 可见性 | 四个分类 **＋** 面板的「过往 Issue」里全部消失 | 「别再让我看见它」是按条目说的，不是按分类说的 |
| 撤销 | 动作条 5 秒内可撤（`UndoBar`） | 与「恢复未读」「批量」共用同一条撤销通道 |

> ⚠️ **不能靠「从归档里删掉」实现丢弃**：归档只是本地留存，远端照样返回这条通知 ——
> 删了归档它会掉回收件箱，用户看到的是「丢弃反而把它叫回来了」。
> 另一条同样踩过的坑：只在「已完成」里过滤丢弃集合 —— 切到「全部」时它会从收件箱冒出来，
> 表现为「丢弃没生效」。两条过滤都收在纯函数 `notifCategoryBase` 里
> （单测 `NotificationCategoryTest` 按「分类 × 本地覆盖」的矩阵钉住）。

> 「丢弃」只出现在**已完成**分类的动作面板里（收件箱分类不给）：放在收件箱会与「完成」抢语义，
> 用户分不清哪个才是「收拾干净」。它在面板里是唯一的危险色动作（`SheetAction(danger = true)`）。
