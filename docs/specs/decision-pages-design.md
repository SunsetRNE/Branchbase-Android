...
事实区：分叉示意（本地绿节点带 `ahead`、远端蓝节点带 `behind`）+ 未推送提交清单（空则「（无数据）」；
**1.1.1 起清单被截断时补一行「…还有 N 条未列出」** —— 引擎的 `unpushed` 最多 50 条而 `ahead` 是全量，
少了这一行就是「把截断画成一共就这些」）。
选项 **1.1.1 起是四条**：保留本地（**推荐，默认选中**）/ **合并远端** / 放弃本地（危险，必须先勾选）/
取消 —— 顺序上「合并」排在「保留」之后是有意的：「推荐」标在无副作用的那一项上，
不把写操作做成按一下就发生的默认路径。
- 只有「放弃本地」会动手 `gitResetHardRemote(repoDir, branch)`；「保留本地」只回一句
  `onResolved("已保留本地提交 · 远端未动")`，**不改任何文件**；
- **「合并远端」（1.1.1）**：把上游 `origin/{branch}` 合进本地（`merge_branch`）—— D11 在 1.0.102
  拆分之后 merge 是允许的（只新增提交、已有 sha 一字不变），而这一页此前只剩「保留 / 放弃」两条，
  用户拿回远端提交的唯一办法是去桌面端。跑动作的是**宿主**（`ui/repository/MergeFlow.kt` 的
  `runMerge`）：合并的执行点全仓库只有那一处，冲突弹窗与冲突详情页在这条路上也接通
  （`LocalPage.MergeConflict`）—— 设置页因此成了**第三个合并宿主**。
...
> —— 改成「远端未动、改动可逐文件查看 / 复制」。
> **1.1.1 再修一次**：那版留下的「App 不做 merge/rebase」在 1.0.102 就过期了，现在的说法是
> 「可以把远端合进本地（merge 只新增提交，已有历史一字不改）」。
**预检**：`hasRemoteRef == false` 时「放弃本地提交」渲染成**禁用行 + 原因**
（「本地还没有 `origin/{branch}` 的记录（未获取过）：先执行一次获取」）；**「合并远端」另有三条**
（1.1.1）—— 仓库停在一次合并里 / 工作区有 N 处未提交改动 / 本地是浅克隆（去加深），
都写成灰掉那一行的理由，而不是点下去才看到引擎拒绝；
`status == null` 时文案是「无法读取仓库状态（目录无效或状态读取失败）」——不再归因「引擎不可用」。
选项分派走**语义枚举**（`ForkChoice`）而不是 `Int` 下标：中间插一项就得人肉对齐所有下标，
错位了不报错、只会让底部按钮去做另一件事（1.1.1 换掉）。
