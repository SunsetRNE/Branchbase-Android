...
>
> **本文是 [`git-mode-design.md`](git-mode-design.md) 的「可视化」子设计**（2026-09 重新分层）：
> 那篇回答「**在哪个页面、什么模式里**用这三棵树，旁边的提交 / 同步 / 合并 / 冲突解决怎么摆」，
> 本文回答「**图怎么画**」（泳道算法、数据模型、分页、配色、性能）。
> 两篇冲突时以 `git-mode-design.md` 的 IA 结论为准；**本文 §8「面板与入口：一份 UI，两份壳」
> 里的宿主清单已被它取代**（入口收敛到「仓库内 Git 模式」，不再四处开花）。
>
...
4. **文件历史要有本地兜底（2026-09 追加）**：浅克隆/未加深时走 REST 并分页；**本地方案要做**
   （`log_file`）—— 已加深的仓库优先用本地历史（离线、无 API 限额），REST 退居兜底；
5. **图谱不设条数上限（2026-09 追加）**：默认 head 全取、「加载更早」不限次数、不设 100/300 的人工上限；
   代价是请求量与本地占用都会长 —— 用**本地持久缓存**换回来（见 §4 的「图谱本地缓存」），
   体积增长**明确接受**，但要给清理入口（见 §4 的清理约定）；
6. **tag 取全量信息（2026-09 追加）**：`name` + 指向的 `sha` + annotated tag 的
   `tagger`（名 / 邮箱 / 时间）与说明；**不是 annotated 时这些字段为空**，不编造。
...
- **取几个 head**：默认分支 + 当前本地分支 + 最近推送分支，各自 `per_page=100` 后合并去重；
  「加载更早」用 `?sha=<窗口内最早的 sha>` 续取 —— **不限次数、不设总量上限**（产品决策 5）。
  没有上限就必须把「加载到哪了」说清楚：列表尾部固定显示「已加载 N 条 · 更早历史未加载」，
  **不许**把截断画成历史尽头（`dangling` 的线头也不许画成断头线）。

### 4.1 图谱本地缓存 = 本地仓库本身（产品决策 5 + 缓存选型 A）

**已拍板（2026-09）**：**不另建 Room / JSON 缓存** —— 应用本身是本地优先的实现，
数据也落在本地：**加深克隆之后，提交就在本地仓库的 `.git/objects` 里**，libgit2 直接读
（离线、无 API 限额、git 自己压缩与 gc）。没有本地副本的仓库，图谱按需从 REST 取并沿用内存级缓存；
想要「打开就秒出」，走 `fetch_deepen` 把历史落到本地。

由此，「占用」= **本地仓库本身的体积**，清理 = 管理本地副本（设置 → 本地仓库 →「管理」页 =
按仓库看占用 / 清理 / 删除，见 [`git-mode-design.md`](git-mode-design.md) §4），
不存在第二份要单独清理的缓存。

> 下面是**当时评估过的三个选项**，保留下来免得以后重新讨论一遍。



不设上限 + 接受体积增长，意味着**不能每次都从头拉**。缓存形态：

| 项 | 约定 |
|---|---|
| 粒度 | 按 `(owner, repo, ref)` 分片；一条提交一行（`sha` 主键去重） |
| 内容 | `sha` · `parents[]` · `subject` · `author` · `date` · `refs[]`（图只需要这些，**不存 diff**） |
| 写入 | 增量 append：每次取到一页就落一页，并记「该分片已取到的最老 sha」（水位） |
| 读取 | 命中即先直出缓存（骨架 → 内容），再后台从 head 续取增量；**head 变了就以 head 为准合并去重** |
| 失效 | 远端 head 变化 · 手动刷新 · 文章级 TTL 都不是必须：提交是**只增不改**的，去重比失效更重要 |
| 存储（**已拍板：不建这份缓存**） | 备选曾是 Room（app 已有缓存库先例 `SearchCacheDatabase`）或 JSON 文件；**现在都用不上** —— 数据落在本地仓库对象库里 |
| 清理 | 设置 → 本地仓库 →「管理」页：**按仓库**看占用并清理（不是所有仓库都大，要能只清一个）；删除本地仓库时顺带清掉派生数据 |

> 阶段 4（加深克隆）之后，图数据的真源就是本地仓库（libgit2 对象）；
> 在此之前（阶段 1–3），没有本地副本的仓库走 REST + 内存缓存。

### 4.2 文件历史的数据源选择（产品决策 4）

```
文件历史请求 ──┬─ 本地仓库已加深 且 该路径在本地历史里  → log_file（本地，离线、无 API 限额）
               └─ 否则（浅克隆 / 无本地副本 / 路径不在本地）→ REST /commits?path=（分页 + 缓存）
```

两条来源的一致性口径与 §4 的合并规则相同：按 `sha` 去重；本地来源**多**出来的东西
（未推送提交、本地重写过的历史）以独立样式标出；UI 不区分「这条来自哪」，
但**必须**在顶部说明当前是本地还是远端来源（离线/分页能力不同）。

## 5. 引擎新增（第 3 阶段，需重建 `.so`）

| 新接口 | 签名（`core/src/git/mod.rs`） | 成功返回 |
|---|---|---|
| 提交图 | `pub fn log_graph(dir: &str, limit: usize, skip: usize) -> Result<String>` | `[{sha, parents[], message, author, time, refs[]}]`（分页：上限由**调用方**按页给，引擎不设总量上限） |
| 加深克隆 | `pub fn fetch_deepen(dir: &str, depth: u32, token: Option<&str>) -> Result<()>` | 空串 |
| tag 列表 | `pub fn list_tags(dir: &str) -> Result<String>` | `[{name, sha, annotated, target_sha?, tagger?: {name, email, time}, message?}]`（非 annotated 时后三项缺省，**不填假值**） |
| 文件历史 | `pub fn log_file(dir: &str, path: &str, limit: usize, skip: usize) -> Result<String>` | 同 `log_graph` 的条目形状（含 `path` 的改名跟随，若 libgit2 给得出） |
...
取该文件的提交序列；每行 → 该次提交里这个文件的 diff（复用 `BranchCompareScreen` 的 diff 渲染，
`BranchDiff.kt`）。数据源**两条**（产品决策 4，选择规则见 §4.2）：

| 来源 | 何时用 | 能力差异 |
|---|---|---|
| `log_file`（本地，**要做**） | 本地仓库已加深，且该路径在本地历史里 | 离线可用、**无 API 限额**、可分页到任意深度 |
| `/commits?path=`（REST，兜底） | 浅克隆 / 没有本地副本 / 路径不在本地历史 | 受 1000 条上限与限流约束；大文件要分页 |

顶部必须说明当前来源（「本地历史」/「远端历史」），并在本地尚未加深时给
「加深克隆」入口（§5 的 `fetch_deepen`，长任务走任务中心）—— 不许让用户以为
「只有这几条」是文件的历史尽头。
...
| **1** | 只读 DAG 图（**含 A 方案的未提交虚节点**）+ 气泡面板入口 + i18n / 文档 / 版本条目 | 阶段 0 | 真机：多分支仓库的图不断线、不串道；虚节点出现/消失不播换页动画 |
| **2** | 引用树 + 文件历史树（REST 分页版）+ 安全动作 + 完整门控 | 阶段 1 | 决策页跳转链路通；危险动作全部落在决策页 |
| **3** | `log_graph(limit, skip)` + `list_tags`（含 annotated 元信息）+ `log_file` + **图谱本地缓存**（§4.1，Room；含清理入口）（重建 `.so`） | Rust 改动 | `cargo test` + `JniSignatureTest`；缓存命中时二次打开不重新拉全量 |
| **4** | `fetch_deepen`（任务中心 + 进度）+ 文件历史**本地兜底**（§4.2）+ 离线图谱（LocalSource 优先）+ 未推送段 | 阶段 3 | 飞行模式下能看图、能看文件历史；加深失败不破坏已有仓库 |
...
### 14.1 已拍板（2026-09，见 §3 决策 4/5/6）

| 问题 | 结论 |
|---|---|
| 文件历史是否要本地 `log -- path` 兜底 | **需要**：已加深走本地（离线、无限额），REST 兜底；见 §4.2 |
| 图谱 head 数 / 条数上限 / 「加载更早」次数 | **都不设上限**；用**图谱本地缓存**（§4.1）抵住请求量与体积，**体积增长接受**但不许没有清理出口 |
| tag 的字段 | **都要**：`name` + `sha` + annotated 的 `tagger` / 时间 / 说明；非 annotated 时留空 |
| 工作区改动要不要画虚节点 | **画（方案 A，2026-09 拍板）**：目标是工作台，多一个浅色节点值得 —— 具体规格见下 |
| 图谱缓存存哪 | **都不建**：本地仓库对象库就是缓存（加深克隆），清理 = 按仓库管理本地副本（见 §4.1） |

**虚节点的规格（方案 A 定稿）**：

| 项 | 约定 |
|---|---|
| 位置 | HEAD **之上**一行（父 = HEAD），随 HEAD 移动 |
| 画法 | 虚线描边 + 半透明填充（与真实提交的实心节点**一眼可分**）；标签 `工作区`；**不画 sha** |
| 数量 | **一个**：把「工作区改动」与「索引已暂存」合成一个「未提交」节点（git 里它们是两件事，但图上分两个节点只会让人以为有两次提交）；**stash 不进图**（显式 stash 还没做，见 §3 决策外的未决项） |
| 出现 / 消失 | `dirty` 从 0 → N 出现、N → 0 消失；元素级淡入淡出 + 尺寸跟随（**不播换页动画**） |
| 点击 | 打开「工作区」视图（面板内的那一档） |
| **不可用**的交互（因为它没有 sha） | 打开提交详情 · 复制 sha · 与其它 ref 对比 · reset / revert / 回滚的目标 —— 图上这些入口对虚节点**置灰并说明原因**，不许点了才失败 |
| 计数 | 节点上带改动数徽标（`repo_status.dirty` 的长度），与顶部状态区同源、同刷 |

### 14.2 仍待拍板

1. 提交图所在的那一档面板算不算 `PageLevel.heavyFirstFrame`（判据是基线：≥3 条慢帧且主段是
   「动画」或「绘制」；`PageTransitions.kt:180-192`）—— 落地后用 perfBeta 基线判，别凭感觉标。
