...
| **提交图** | 泳道 DAG + **未提交虚节点** + 分页脚注 + 点一行看这条提交的 diff（1.0.99；**本地读不出来时同一屏换成远端提交详情**，1.1.7）+ **未推送段**（1.0.101：本地来源下逐条标「未推送」） | **本地 `log_graph`**（本地仓库存在且**不是浅克隆**时，离线、看得见未推送的提交，并逐条带 `unpushed`）；否则 REST `/commits?sha=&per_page=100`（**保留 `parents`**）兜底 —— REST **首页** 1.1.7 起进既有 `PageCache`（键 `detail:graph:owner/repo@branch`；本地副本是浅克隆时宿主还会**预热**一次） | 已落地（1.0.94 REST 版 → **1.0.98 换成本地优先** → **1.0.101 标未推送段** → **1.1.7 REST 首页缓存 + 提交行退到远端详情**） |
...
**提交行看 diff 的退路**（1.1.7）：本地仓库是**浅克隆**时（`clone` 只拿 depth=1 那一条历史），
`diff_commit` 要算「这次提交 vs 它的父」，而那个父对象**根本不在本地** —— 引擎只能报错，
提交图里点任何一条历史提交，看到的都是一句「读取失败」（真机日志里那五条
「本地 diff ▸ 读取失败（2197d5d…）」就是这个根）。所以 `LocalDiffScreen` 在
**本地读不出来 + 知道 `owner`/`repo`/`sha`** 时，整屏换成 `CommitDetailScreen`
（REST 的提交详情，自带 `PageCache` 直出与重试）；本地**读得出来**时一律用本地那一份（含工作区档）。
判定放在**真正失败的那一刻**，而不是「按图的来源选目的地」：来源只说明图从哪来，
本地图读得出来时照样可能算不出某一次提交的 diff（浅克隆边界那一条），两头各判一次必然对不上。
...
| 上游… | `nav_upstream` 上游 / Upstream | `!git.exists`；**自持仓库整枚不画**（1.1.5，见下） | `UpstreamSetupScreen`（上游**自动探测** + 远端 URL；可能转分叉页） |
...
**上游 = 自动探测出来的事实，不是让用户填的输入**（1.1.5，D-p）。1.1.3 之前这一出口只有
一张手填表（远端地址 + 上游分支名），可这两件事里**只有一件是用户的自由**：往哪推是用户的选择，
「这个仓库有没有上游、上游是谁、上游还在不在」是 GitHub 已经写着的事实 —— 让用户回忆事实，
真机上的三种现场都出现过：填成自己的仓库（推了个寂寞）、填成上游（对别人的仓库没有写权限）、
`https://github.com/<repo>.git` 少了 owner（1.1.3 的默认值就是这么拼的，1.1.5 一并修）。

所以 1.1.5 把这一页拆成两件事：**上游关系**（自动探测，只用来给一句话）与**推送远端**
（仍然是本仓库自己的地址 —— 复刻推的必须是自己的复刻）。六种状态与它们的证据：

| 状态 | 证据（仓库详情 `/repos/{owner}/{repo}` 优先，GraphQL 关系态兜底） | 界面 |
|---|---|---|
| 自持仓库 | `fork=false` 且没有 `parent` | **整枚出口不画**（`UpstreamRelation.showsEntry=false` → `onUpstream = null` → 面板不画胶囊），弹窗只留状态句 |
| 有上游仓库 | `parent.full_name` 有值，且探得到、未归档 | 出口照常；页面「上游状态」卡给出上游名与一句「可拉取 / 可比对 / 可开 PR」 |
| 上游已归档 | 同上，但 `archived=true` | 同上；说明改成「只读：不能推、不能再开新 PR」 |
| 上游已删除 | `fork=true` 但 `parent` 没了 | 说明改成「原上游地址不再可用，本仓库现在自己就是网络的源」 |
| 上游已私有化 | `parent` 还在，`/repos/{parent}` 回 **404** | 说明改成「关系还在，但没有访问权：拉取 / 比对 / 开 PR 都会失败」 |
| 上游未知 | 仓库详情没读到、没令牌、或探测本身没打通 | 出口照旧、**不藏功能**；说明如实说「判断不了」 |

三条口径，都是「宁可少说一句，不要替用户下结论」：

1. **「已删除」与「已私有化」能分开，靠的是 GitHub 自己的复刻语义**，不是猜 404：
   父仓库被删、或公共仓库转私有，都会让复刻**断开成独立网络**（`parent` 随之消失）；
   反过来 `parent` 还在却读不到，只剩「没权限」这一种解释（官方文档
   *What happens to forks when a repository is deleted or changes visibility*）。
   所以判定顺序是**先看复刻关系在不在，再看能不能读到**。
2. **没有令牌就不探、不猜**：没令牌读不到私有仓库，那是本地状态，不是上游的状态。
   探测请求本身没打通（超时 / 限流）同样收敛成「未知」—— 拿它去说「已私有化」，
   用户会看到一个自己并没有的结论。
3. **只有自持仓库藏出口，其余五态一律留着**：探测不到不等于没有上游，
   隐藏功能比多说一句坏得多（钉子：`UpstreamRelationTest.只有自持仓库藏出口_其余五态都留着`）。

**别处的上游入口一律不动**：设置 →「本地仓库」那一页、「分支同步」页未跟踪分支上的
「设为上游并推送」都照旧 —— 本地仓库可能是 `git init` 出来的（根本没有复刻关系），
那条推送路径与复刻网络无关，不在这次收窄的范围内。
...
| D-i | **不另建图谱缓存**（Room / JSON 都否掉）：本地仓库对象库就是缓存 | §6.3；说的是**本地图** —— 远端**首页** 1.1.7 起确实落进了既有 `PageCache`（见 D-q） |
| D-j | **「引用树」档只读**：分支的切换 / 新建 / 删除不搬进浮层（有后果 → 决策页），落点是既有的分支管理 / 本地分支同步；这一档只回答「有哪些引用、我在哪、哪些只在远端」 | §2 / §3.2（1.0.95） |
| D-k | **改动清单 / 提交行的落点 = 独立只读的「本地 diff 页」**（不给文件查看器加「本地工作树」来源）| §3.2 / §11.6；1.0.99；**本地读不出来时同一屏退到远端提交详情**（1.1.7） |
...
| D-p | **「上游」= 自动探测出来的事实，不是让用户填的输入**：六态（自持 / 有上游 / 已归档 / 已删除 / 已私有化 / 未知）各给一句针对性说明；**自持仓库整枚出口不画**（`onUpstream = null` → 面板不画那枚胶囊），其余五态一律保留（探测不到不等于没有上游）；判定顺序「先看复刻关系在不在，再看能不能读到」，**没令牌或探测本身没打通一律收敛成「未知」**，不拿 404 猜「已私有化」。推送目的地仍是本仓库自己的复刻 —— 探测结果只用来给一句话 | §3.6；`ui/repository/UpstreamRelation.kt`（纯函数 + 21 例单测）与 `RepoActions.loadUpstream`（`getJson` 而非 `getRepoInfo`：HTTP 状态必须留在 `ERROR:HTTP 404: …` 里）；1.1.5 |

| D-q | **提交图的 REST 首页进既有 `PageCache`，并在浅克隆时预热**：不新造缓存层（复用 `SearchCacheDatabase` 那一套），缓存键与 URL 只在 `CommitGraphCache` 拼一份（面板取数与宿主预热差一个字符就是「明明暖过还是慢」，且完全静默）；边界三条 —— **只首页**、**只 REST**、**回源失败不换屏** | §3.2 / §6.3；1.1.7 |

| D-r | **「没有上游」这件事要一致地不画**：工作区档那行「未设置上游」与「上游…」出口用**同一份**探测结果、但**不是同一个开关**（`showsUpstreamLine` vs `UpstreamRelation.showsEntry`）—— 出口看「有没有上游」，那一行还要看**当前分支有没有跟踪上远端**：跟踪上了就必须画（那是「你跟踪的是 `origin/main`」这个事实），没跟踪且自持才不画；`rel` 为 null（还没探到）**不许藏** | §3.2；`UpstreamRelation.kt`；1.1.7 |
...
git 自己压缩与 gc）。没有本地副本的仓库，图谱按需从 REST 取；
**1.1.7 起 REST 的首页落进既有的 `PageCache`**（键 `detail:graph:owner/repo@branch`，与提交详情
同一个库、同一个类型，见 `CommitGraphCache`）—— 这一条针对的正是「本地有副本、但它是浅克隆」
那种最尴尬的处境：图谱注定走 REST，却每次打开、每次换档回来都白等一次网络。边界写死三条：
**只缓存首页**（翻页的键与首页不同，不缓存，免得两套形状互相覆盖）、
**只对 REST 生效**（本地来源在本机读，读一次比查缓存快，缓存它反而会把加深前的图留在屏幕上）、
**回源失败不换屏**（缓存已经出过内容时失败只记一条日志，屏上保留缓存那一份）。
想「打开就秒出」的正路仍是 `fetch_deepen` 把历史落到本地。
