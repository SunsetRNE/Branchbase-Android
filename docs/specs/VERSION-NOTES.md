...
**每一版改了什么、为什么这么改**都在这份文档里 —— §二 `versionName` 条目（1.0.22 → **1.1.5**）
与 §三 `versionCode` 流水（129 → **211**）。写法样板也在下面（1.1.1 从那个文件搬进来的）。
...
- **211**：Git 模式「上游」改为自动探测（六态：自持 / 有上游 / 已归档 / 已删除 / 已私有化 / 未知）；
自持仓库整枚出口不画；顺带修掉上游页默认地址缺 owner（一次提交，故 +1）
