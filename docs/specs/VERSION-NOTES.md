...
> `version.properties` 在 1.1.1 收瘦过一次：它原来还留着三个写法样板（约 40 行）与三条
> versionCode 样板，现在**全部搬到这里** —— 那个文件的职责只剩「两个值 + 指路」，
> 改版本号时不必在一堆历史注释里找那两行。
...
### 写法样板（原先在 `version.properties` 里，1.1.1 搬来）

**样板 1（分点式：一版多件修复，接线缺陷类）—— 1.0.34：消息页「长按 → 单条动作面板」状态机修复 + 批量失败回滚范围修正**

① 接线缺陷：`NotificationList` 的 `onLongClick` 一直传 `enterSelection`，而 `sheetTarget` 只有
多选条「更多」会赋值 —— 快捷动作面板的非多选分支从引入起就是死代码，长按退化成「进多选」，
README 与 `design/messages-redesign` 原型描述的「长按出单条动作」从未生效。改为
`onLongClick = { sheetTarget = it }`，多选回到它该在的位置（面板末尾的选项）。
② 修好接线后暴露：面板里的单条「标记完成」只写本地、从不调 `markNotificationDone`（该接口此前
只在批量路径里用过），刷新后条目原样回来。抽出 `markDoneRemote`（本地乐观归档 → 远端 DELETE →
失败按 id 回滚）并接到单条路径上。
③ 批量失败改为**只回滚失败的条目**：整批回滚会把远端已改成功的条目在本地撤回，用户看到
「批量失败」、刷新后一部分又自己变已读，而这正是「回滚为了跟远端一致」的反面。规则抽成纯函数
`bulkRollbackTargets`，新增 `NotificationBulkRollbackTest` 6 例。失败项保留选中以便直接重试；
静音不回滚也不给撤销（本地无可见状态可退）。
④ 退出多选不再重置 `bulkRunning`：批量是仍在跑的远端长任务，旧实现把两者绑定，退出后能再触发
一批（两批并发打远端，120ms 限流间隔失效），且旧批次收尾的 `exitSelection()` 会清掉用户新选的
一批。现按 `bulkSeq` + 选择是否被改过决定收尾。
⑤ 点击已读消息不再重复发远端写请求（GitHub 同秒写请求有二级限流）。

**样板 2（功能型：先讲为什么，再给实测数字，末尾带顺带修的）—— 1.0.40：慢帧守望（帧级定位）**

「切页那一下卡了多少毫秒、卡在哪一段」原先只能靠开发者选项 + `adb shell dumpsys gfxinfo <pkg>
framestats`，而那条路有三个绕不开的硬伤：只保留最近 ~120 帧（90Hz 约 1.3 秒，人去点永远慢半拍）、
dump 自身跑在被测量进程的主线程（测一次就自己造一根柱子）、取数要另开终端/分屏且还得防着被测
App 被 cached app freezer 冻住。现在把测量搬进 App 自己：
`Window.addOnFrameMetricsAvailableListener` 取每帧分段（等待/输入/动画/布局/绘制/上传/下发/交换，
与 framestats 同源），≥32ms 记一条明细、每 60s 记一条小结，页面归属取「最近一条 UI 日志」
（不另造「当前页面」状态）。首轮实测已能直接定位：进设置页 95.9ms（动画 43.7 / 绘制 45.5）、
进日志页 113.8ms、仓库详情页数据到达后一帧 198.1ms（动画 129.0）—— 而布局恒 0~2ms、输入恒 0，
说明瓶颈在「首次组合 + 首次绘制」，不在布局。顺带修掉两处：日志写盘其实是**覆盖写**
（`FileAppender` 用了截断模式，磁盘上永远只剩最近一批，而「设置 → 日志 → 导出 .log」读的正是
这个文件）；慢帧日志把自己当成了「页面」，日志里套了五层。设置页：行首图标与右端的值改按整行
垂直居中（它们被内层 `Row` 的默认 Top 对齐顶到行顶，说明折行后尤其明显）。

**样板 3（工程/文档治理型：改的是政策与仓库结构）—— 1.0.38：设计文档重组：草图与结论分家**

原先 `.gitignore` 里 `/design/`（草图）与 `/docs/`（文档）是**一起忽略**的，代价是
`core/src/html/mod.rs` 长年引用一份 `docs/html-parser-design.md`，而它从未进过版本库、丢失后
无从找回（注释指向不存在的文件比没有注释更坏）。现在按「这个结论半年后还有效吗」一刀切：
`/docs/` 入库（README 收纳规则 + `specs/` 重点文档），`/design/` 仍只放草图、永久不入库。补回
`html-parser-design.md`（按 `matcher.rs` 实际行为与 16 个单测写，章节号对齐代码里既有的
§3 / §5.1 两处锚点）；`NAVIGATION-NOTES` 与 `BUILD-NOTES` 从根目录移入 `specs/`；三份原型
「说明与落实方案」抽入库副本到 `specs/prototypes/`（草图本体仍不入库，另加互指）。README 项目
结构块与文档政策随之重写。
...
**Git 工作台三处收口：没有上游就不画「未设置上游」、提交图的 REST 首页不再白等一次网络、点一条提交读不出本地 diff 时换远端详情。**

① **「未设置上游」跟着上游判定走**（用户报的）：工作区档第二行原本写死「有跟踪写分支名，否则写『未设置上游』」——
自持仓库（本仓库就是复刻网络的源）**没有上游可设**，那句话落在那里是一句永远无法执行的催促。
新增纯函数 `showsUpstreamLine(hasUpstream, upstreamBranch, rel)`：跟踪上了就画（那是「你跟踪的是
`origin/main`」这个事实），没跟踪且判成自持才不画。它与藏「上游…」出口的 `UpstreamRelation.showsEntry`
**同源但不是同一个开关** —— 出口只看「有没有上游」，这一行还要看当前分支有没有跟踪上远端；
`rel` 为 null（还没探到）**不许藏**，拿「不知道」当「没有」会让真有上游的仓库失去那句唯一提示。

② **提交图：REST 首页进缓存 + 浅克隆时预热**（用户报的「本地已有仓库，加载提交图仍然很慢」）：
提交图在浅克隆里注定走 REST（`log_graph` 撞缺掉的父对象，1.1.4 的现场），而此前**没有任何缓存** ——
每次打开这一档、每次换档回来、每次 `refreshTick` 变化都重新发一次 `/commits`。
新增 `ui/repository/CommitGraphCache.kt` 与 `PageCache.graphKey(owner, repo, branch)`
（键 `detail:graph:o/r@branch`）：首页走 `PageCache.refreshDetached`（新鲜直接命中、过期先直出再回源），
宿主在**本地副本是浅克隆**时后台 `warm` 一次（那时注定走 REST，等于把那次网络提前到用户没在等的时刻）。
边界三条写死：**只首页**（翻页的键与首页不同，不缓存）、**只 REST**（本地来源在本机读，
读一次比查缓存快，缓存它反而会把加深前的图留在屏幕上）、**回源失败不换屏**（缓存已出过内容时只记一条日志）。
缓存键与 URL 只在 `CommitGraphCache` 拼一份 —— 面板取数与宿主预热各拼一次，差一个字符就是
「明明暖过还是慢」，而且完全静默。

③ **点一条提交：本地读不出来就换远端详情**（用户报的「加载提交图，无法加载详细的变更内容」）：
点一行走 `RepoRoute.LocalDiff(sha)` → `LocalDiffScreen` → `diff_commit` 要算「这次提交 vs 它的父」，
浅克隆里那个父对象**根本不在本地**，引擎只能报错 —— 屏上就是一句「读取失败」（真机日志里那五条
「本地 diff ▸ 读取失败（2197d5d…）」同根）。现在 `LocalDiffScreen` 在**本地读不出来 + 知道
`owner`/`repo`/`sha`** 时整屏换成 `CommitDetailScreen`（REST 的提交详情，自带 `PageCache` 直出与重试）；
本地**读得出来**时一律用本地那一份（工作区档不受影响）。判定放在**真正失败的那一刻**，
而不是「按图的来源选目的地」：本地图读得出来时照样可能算不出某一次提交的 diff（浅克隆边界那一条），
两头各判一次必然对不上。

④ **钉子**：`UpstreamRelationTest` 新增 5 例（自持 + 没跟踪 → 不画；有上游 / `rel` 为 null / 未知 → 照画；
跟踪上了自持也照画；`hasUpstream=true` 但分支名是空 / 空白不算跟踪）= **26 例**；
新增 `CommitGraphCacheTest`（4 例：首页地址按分支取、翻页按提交 sha 取、空分支不写 `sha=`、
缓存键按仓库与分支分开且不跟提交详情撞键，并把「页大小只有一份」钉成 `GRAPH_PAGE_SIZE == 100`）。
口径补进 `docs/specs/git-mode-design.md` §3.2 / §6.3 与台账 D-i / D-k / D-q / D-r。
...
**Git 模式阶段 5 收口：两条入口（分叉页「合并远端」/ PR 冲突「拉到本地解决」）+ 一轮对账修账。**

① **对账先于动手**：这一版开工前把 `git-mode-design.md` 逐条对回代码，捞出四类「文档说行、代码说不行」
   的问题，全部在本轮修掉（不是新增功能，是账没对上）：
   - **引擎并不认远端分支名**（§7 那句「引擎已支持远端分支名」是错的）：`resolve_merge_target` 只认
     **裸分支名**，按 `refs/heads/{x}` → `refs/remotes/origin/{x}` 的顺序找。而「本地与上游分叉」的
     场景里两边**同名**（都叫 `main`）—— 传裸名会解析到本地那条 = HEAD 自己，`merge_analysis` 判
     `up_to_date`，表现是「点了合并，什么都没发生」且**不报错**。现在显式远端名（`origin/x`）先按
     `refs/remotes/{name}` 解析；裸名语义一字未改（带斜杠的本地分支名不会被抢走）。
     仍要重建 `.so`，否则真机跑的还是旧行为；
   - **`合并` 这个日志锚点没登记进 `LOG_ANCHORS`**：`MergeFlow.kt` 的注释写着「已登记」，实际只有
     `PR合并`。而 `LogAnchorsTest` 只做「表 → 源码」单向检查，抓不到漏登记 —— 现在补登记，并把
     钉子做成**双向**（源码里每个 `*LOG_TAG` 常量都必须在表里）；
   - **分叉页文案还写着「App 不做 merge / rebase」**（`note_diverged_explain` /
     `note_keep_local_workspace`）：D-g 在 1.0.102 就放行了 merge，这两句一直没跟着改；
     面板底部那句「提交 / 推送 / **合并**入口按阶段接入」也过期了（合并胶囊 1.0.103 就在）；
   - **`tools/i18n/extract.py` 已经跑不起来**：`strings.tsv` 有 7 个字面量重复（3 条是 1.0.99/1.0.100
     留下的，4 条是 1.0.103 加的），而工具的加载器遇到重复直接 `SystemExit`。`check-i18n` 只校验
     XML 键集合，所以 CI 一直是绿的。现在按「一个官话字面量 = 一个资源」把它们并成 7 条，
     工具恢复可用（本轮所有新字符串都是它抽的）。

② **入口 ①：分叉页第三条「合并远端」**（设置 → 本地仓库 → 更新/推送被判分叉时的那一页）。
   选项从三条变四条：**保留本地（推荐，默认选中）/ 合并远端 / 放弃本地（危险）/ 取消**。
   - 三条前置写成**灰掉那一行的理由**（工作区脏 N 处 / 仓库停在一次合并里 / 浅克隆去加深），
     而不是点下去才看到引擎拒绝；
   - 跑动作的是**宿主**（`MergeFlow.runMerge`）：合并的执行点全仓库只有那一处；
   - 于是设置页成了**第三个合并宿主** —— 冲突弹窗与冲突详情页在这条路上也接通
     （`LocalPage.MergeConflict`），否则「点了合并、弹出冲突」之后无处可去；
   - 顺带把「选项下标」换成语义枚举（`ForkChoice`）：这一页原来是 `option == 1` 的分派，
     中间插一项就得人肉对齐所有下标，错位了不报错、只会让底部按钮做另一件事。

③ **入口 ②：PR 冲突的「拉到本地解决」**（PR 详情页，`mergeable == false` 时的第二枚入口）。
   点开的是**预选了 PR head** 的合并决策页 —— 那个分支常常本地与远端跟踪引用都没有（从没 fetch 过），
   所以合并页新增「预选不在清单里就补一条只在远端的」（引擎本来就会自己 fetch 一次）。
   两条如实置灰的理由：**本地没有这个仓库的副本**；**head 在复刻仓库里**（本地副本的 origin 上没有
   它，引擎只从 origin 拉）—— 判据取自 `head.repo.full_name`，缺键时**不预判**（与 `mergeable == null`
   同一条口径）。「合并此 PR」与「拉到本地解决」视觉上分开：前者是在 GitHub 上合掉，后者是先拉下来
   把冲突解掉，混成一个按钮会让人以为点了就会合并。

④ **`version.properties` 收瘦**：三个写法样板（约 40 行）与三条 versionCode 样板全部搬进本文档，
   那个文件只剩格式契约、指路与两个值 —— 改版本号时不必在历史注释里找那两行。

验证：`cargo test` 108 例（+3 条：显式远端名合的是远端那条 / 带斜杠的本地分支不被抢走 / 拉不到时
说的是远端找不到）· `:app:testDebugUnitTest` 923 → **939** 例（`SyncDecisionPrecheckTest` +6、
`PullDetailModelsTest` +5、`MergeModelsTest` +3、`GitWorkbenchWiringTest` +1、`LogAnchorsTest` +1）
· `assembleDebug` · `check-i18n --min-coverage 100` · **重建 `.so`**。
取证：`extract.py` 的重复字面量是**硬错误**（不是警告），所以「表里多写一条」会让整条抽取链停摆 ——
`check-i18n` 查不出来，只有真去跑一次抽取才会发现。
...
>
> 写法样板（原先在 `version.properties` 里，1.1.1 搬来）——一行 = 版本码 + 这一版改了什么，
> 末尾带上「为什么只 +1」：
>
...
点一条提交读不出本地 diff 时换远端详情（`CommitDetailScreen`）（一次提交，故 +1）
...
+ `合并` 日志锚点补登记 + `LogAnchorsTest` 改双向 + 分叉页三条过期文案（「App 不做 merge」）
+ `extract.py` 恢复可用（`strings.tsv` 7 个字面量重复并掉）+ `version.properties` 收瘦
+ 对账修账：`GitRefsModelsTest` 5→8、`GitWorkbenchWiringTest` 14、写方法表 15 条
+ `SyncDecisionPrecheckTest` +5、`PullDetailModelsTest` +5、`MergeModelsTest` +3（一次提交，故 +1）
