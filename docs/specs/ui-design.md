...
> **后续补充（2026-09）—— 上表约束的是 `border` 本身，不是「一切描边」。**
>
> 首页 / 个人页三 Tab（概览 · 仓库 · 动态）/ 设置页的卡片框与分隔线后来改走**新角色**
> `Primer.BorderEmphasis`（浅 `#000000` / 深 `#30363D`）；设置页的可交互控件边界
> `Primer.BorderControl` 浅色也一并拉到 `#000000`（深色仍 `#6E7681`）。
>
> 这与本节结论**不冲突**，但别把两件事混起来：
>
> - **拉黑的只有浅色**。深色下 `BorderEmphasis` 与 `border` 同值 —— 纯黑压在 `#0D1117` /
>   `#161B22` 上只有约 1.1:1，会直接消失，所以深色板逐位未动；
> - **拉黑的只有一个子集**。其余 24 个文件、55 处 `border` 引用仍是 `#BFC1C9`，
>   上面「退回 wireframe」的判断对它们依然成立；
> - 换言之，现在是「**少量强调位置拉黑 + 其余保持弱描边**」，而不是全局改风格。
