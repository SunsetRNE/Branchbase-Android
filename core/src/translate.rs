...
//!
//! 因此默认走 MyMemory；接口本身是**可替换的**：只要改 [ENDPOINT] 与
//! [parse_response]，上层（Kotlin 分片与缓存、页面里的 JS）完全不用动。
//!
//! ## 约束（由上层负责，不在这里做）
//!
//! MyMemory 单次 `q` 上限 500 字符，且是「一次请求一段」。分片、去重、串行与缓存
//! 都在 Kotlin 侧（`ui/repository/Translator.kt`）——放上层才能跨页面复用缓存，
//! 也避免这里为「一批文本」引入并发与重试策略。
